...
const querySchema = Joi.object({
  ...
  filter: Joi.object({
    customer_username: Joi.alternatives()
...
const put_customer = async (req, res) => {
  try {
    ...
    const existingCustomer =
      value.customer_email || value.customer_nohp
        ? await tbl_customer.findOne({
            where: {
              [Op.and]: [
                {
                  [Op.or]: [
                    ...(value.customer_email
                      ...
                      : []),
                    ...(value.customer_nohp
                      ? [{ customer_nohp: value.customer_nohp }]
                      : []),
                  ],
                },
    ...
    const hashedPassword = value.customer_password
      ? await bcrypt.hash(value.customer_password, saltRounds)
      : update_customer.customer_password;

    await update_customer.update({
      customer_username:
        value.customer_username || update_customer.customer_username,
      customer_full_name:
        value.customer_full_name || update_customer.customer_full_name,
      customer_nohp: value.customer_nohp || update_customer.customer_nohp,
      customer_address:
        value.customer_address || update_customer.customer_address,
